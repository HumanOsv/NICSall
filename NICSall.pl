...
sub G03Input {
	...
	if ( ($option_nbo == 0) && ($option_pseudo == 0) ) {
		print COMFILE "#p $Header nosymm nmr(PrintEigenVectors) guess=Huckel geom=Connectivity \n";
	}
	if ( ($option_nbo == 1) && ($option_pseudo == 0)) {
		print COMFILE "#p $Header nosymm nmr(PrintEigenVectors) guess=Huckel geom=Connectivity pop=nbo6read \n";
	}
	if ( ($option_nbo == 0) && ($option_pseudo == 1) ) {
		print COMFILE "#p $Header pseudo=read nosymm nmr(PrintEigenVectors) guess=Huckel geom=Connectivity \n";	
	}
	if ( ($option_nbo == 1) && ($option_pseudo == 1) ) {
		print COMFILE "#p $Header pseudo=read nosymm nmr(PrintEigenVectors) guess=Huckel geom=Connectivity pop=nbo6read \n";
	}
	#
...
foreach my $type_g (@TypeGraph) {
	if ( $type_g == 1 ) {
		...
		print "ERROR Please choose type graphic: \n";
		print "\t 1 = Isotropy     \n";
		print "\t 2 = Anisotropy   \n";
		print "\t 3 = Component XX \n";
		print "\t 4 = Component YY \n";
		print "\t 5 = Component ZZ \n";
		print "\t 6 = FiPC         \n";
		print "\t 7 = Scan         \n";
		print "\t 8 = FiPC & Scan  \n";
		print "\t 9 = Symmetry Properties of the Shielding Tensor \n\n";
		exit (1);
	}
}
#
...
if( scalar (@cube_out) > 0 ) {
	system("mv BOX.vmd *.txt *.cube *.vtk *.vti ./Resources/. 2>/dev/null");
} else {
	system("mv BOX.vmd *.txt *.cube *.vtk *.vti ./Resources/. 2>/dev/null");
}
#
